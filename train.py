...
model_G.load_state_dict(torch.load('./checkpoint/init_model_1.pth'))
...
for e in range(0, opt.epoch):
    ...
    for i, inputs in pbar:
        ...
        anime = inputs['anime'].cuda()
        anime_gray = inputs['anime_gray'].cuda()
        anime_smooth = inputs['anime_smooth'].cuda()
        ...
        if e < opt.init_epoch:
            ...
            anime_D = model_D(anime)  
            generated_D = model_D(generated)  
            anime_gray_D = model_D(anime_gray)     
            anime_smooth_D = model_D(anime_smooth)            

            GP = gradient_penalty(anime, generated, D=model_D)

            loss_D = losses.loss_D(anime_D, generated_D, anime_gray_D, anime_smooth_D) + GP
            loss_D.backward()
            optimizer_D.step()
            ...
            loss_G = losses.loss_G(generated_D, real, generated, anime_gray)
            loss_G.backward()
            optimizer_G.step()
        ...
        with torch.no_grad():
            generated = model_G(real)
            torchvision.utils.save_image(torch.vstack([real, generated]), f'./{e}.png', normalize=False, nrow=6)
